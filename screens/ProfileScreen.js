...
const ProfileScreen = () => {
  ...
  useEffect(()=>{
    const fetchUserProfile = async()=>{
      try{
          const response  = await axios.get(`http://192.168.1.5:8000/profile/${userId}`)
...
//for fetch orders
  useEffect(()=>{
    const fetchOrders = async()=>{
      try{
        const response = await axios.get(`http://192.168.1.5:8000/orders/${userId}`)
